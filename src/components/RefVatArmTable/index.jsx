...
const RefVatArmTable = ({isAuth}) => {
    ...
    const save = async (id) => {
        try {
            ...
            if (index > -1) {
                ...
                api.editRefVatArm({ id: newData[index].id, startdate: newData[index].startdate, enddate: newData[index].enddate, vatrate: newData[index].vatrate }, isAuth).then(res => res.json()).then(data => {
                    console.log(data);
                })
    ...
    const handleDelete = (id) => {
        ...
        data.forEach((item) => {
            item.id === id && api.deleteRefVatArm({ id: item.id }, isAuth).then(res => res.json()).then(data => {
                console.log(data);
            })
        })
    };
    ...
    const onFinish = (values) => {
        ...
        api.createRefVatArm({ startdate: startdate, enddate: enddate, vatrate: values.vatrate }, isAuth).then(res => res.json()).then(data => {
            console.log(data);
            if (data === 1) {
                setPopup(false);
                fetchData();
